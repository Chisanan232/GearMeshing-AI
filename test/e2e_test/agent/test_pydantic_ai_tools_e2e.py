...
class TestPydanticAIToolsE2E:
    ...
    def agent_settings(self) -> AgentSettings:
        ...
        model_settings = ModelSettings(
            customized_name="test-agent", provider="test", model="test-model", temperature=0.7, max_tokens=2000
        )
    ...
    @pytest.mark.asyncio
    async def test_agent_read_file_with_prompt(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, test_content)
            or fuzzy_match(output_str, "success")
            or fuzzy_match(output_str, "Hello")
        )

    @pytest.mark.asyncio
    async def test_agent_write_file_with_prompt(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "write")
            or fuzzy_match(output_str, "success")
            or fuzzy_match(output_str, "output.txt")
        )

    @pytest.mark.asyncio
    async def test_agent_list_files_with_prompt(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_agent_run_command_with_prompt(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "hello") or fuzzy_match(output_str, "command") or fuzzy_match(output_str, "success")
        )

    @pytest.mark.asyncio
    async def test_agent_with_all_tools_registered(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "Initial data")
            or fuzzy_match(output_str, "data.txt")
            or fuzzy_match(output_str, "success")
        )

    @pytest.mark.asyncio
    async def test_agent_tool_output_parsing(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "success")
            or fuzzy_match(output_str, "read_file")
            or fuzzy_match(output_str, "content")
        )

    @pytest.mark.asyncio
    async def test_agent_handles_file_not_found(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "not found")
            or fuzzy_match(output_str, "error")
            or fuzzy_match(output_str, "nonexistent")
        )

    @pytest.mark.asyncio
    async def test_agent_sequential_file_operations(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "Content from file 1")
            or fuzzy_match(output_str, "file2")
            or fuzzy_match(output_str, "write")
        )

    @pytest.mark.asyncio
    async def test_agent_with_encoding_specification(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
    ...
    async def test_agent_tool_integration_workflow(
        ...
        assert (
            fuzzy_match(output_str, "read_file")
            or fuzzy_match(output_str, "write_file")
            or fuzzy_match(output_str, "success")
        )

    @pytest.mark.asyncio
    async def test_agent_tool_with_optional_parameters(
        self, adapter: PydanticAIAdapter, agent_settings: AgentSettings, temp_dir: str
    ) -> None:
        ...
        assert (
            fuzzy_match(output_str, "list_files")
            or fuzzy_match(output_str, "success")
            or fuzzy_match(output_str, "files")
        )
