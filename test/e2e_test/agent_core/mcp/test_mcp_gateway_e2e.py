...
class TestMCPGatewayIntegration:
    ...
    async def test_gateway_health_and_status(self, gateway_client: GatewayApiClient):
        ...
        if gateways:
            for gateway in gateways:
                ...
                logger.info(f"Gateway instance: {gateway.id} (enabled: {gateway.enabled}, reachable: {gateway.reachable})")
    ...
    async def test_complete_gateway_workflow(self, gateway_client_with_register_servers: GatewayApiClient):
        ...
        logger.info(f"Complete workflow verified: {len(catalog_response.servers)} servers, {len(tools_response.data)} tools")
    ...
    async def test_gateway_server_filtering(self, gateway_client_with_register_servers: GatewayApiClient):
        ...
        logger.info(f"Filtering test passed: {len(all_servers.servers)} total servers, {len(active_servers.servers)} active")
