    ...
    async def test_approval_required_workflow(
        ...
        try:
            ...
            if result.status == WorkflowStatus.AWAITING_APPROVAL:
                ...
                assert "deploy" in approval_request.description.lower(), "Approval request should mention deployment"

                # Test approval
                approval_result: WorkflowResult = await orchestrator_service.approve_workflow(
                    workflow_id=workflow_id, decision="APPROVE", comment="Approved for smoke test"
                )

                # Wait for approval completion
                final_result: WorkflowResult = await workflow_helper.wait_for_workflow_completion(
                    initial_result=approval_result, timeout_seconds=120, poll_interval=5.0
                )
    ...
    async def test_rejection_with_alternative(
        ...
        try:
            ...
            if result.status == WorkflowStatus.AWAITING_APPROVAL:
                ...
                final_result: WorkflowResult = await workflow_helper.wait_for_workflow_completion(
                    initial_result=rejection_result, timeout_seconds=120, poll_interval=5.0
                )
        ...
        try:
            ...
            if result.status == WorkflowStatus.AWAITING_APPROVAL:
                ...
                cancel_result: WorkflowResult = await orchestrator_service.cancel_workflow(
                    workflow_id=workflow_id, reason="Cancelled for smoke test"
                )
        ...
        try:
            ...
            if result.status == WorkflowStatus.AWAITING_APPROVAL:
                ...
                if hasattr(final_result, "execution_history") and final_result.execution_history:
                    approval_events = [
                        event for event in final_result.execution_history if event.get("event_type") == "approval"
                    ]
