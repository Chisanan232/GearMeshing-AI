...
class WorkflowAssertions:
    ...
    def assert_workflow_success(result: WorkflowResult) -> None:
        ...
        """
        assert result.status == WorkflowStatus.SUCCESS, f"Expected workflow to succeed, but got status: {result.status}"
        assert result.output is not None, "Successful workflow should have output"
        assert result.run_id is not None, "Workflow should have an ID"

    @staticmethod
    def assert_workflow_failure(result: WorkflowResult, expected_error: str | None = None) -> None:
        ...
        """
        assert result.status == WorkflowStatus.FAILED, f"Expected workflow to fail, but got status: {result.status}"
        assert result.error is not None, "Failed workflow should have error message"
    ...
    def assert_workflow_awaiting_approval(result: WorkflowResult) -> None:
        ...
        assert result.approval_request is not None, "Workflow awaiting approval should have approval request"
        assert result.run_id is not None, "Workflow should have an ID"
    ...
    def assert_workflow_cancelled(result: WorkflowResult) -> None:
        ...
        assert result.error is not None, "Cancelled workflow should have error message"
        assert "cancelled" in result.error.lower(), f"Error message should mention cancellation: {result.error}"
    ...
    def assert_workflow_execution_history(result: WorkflowResult, min_nodes: int = 2) -> None:
        ...
        assert len(history) >= min_nodes, f"Expected at least {min_nodes} execution nodes, got {len(history)}"
    ...
    def assert_approval_request_structure(approval_request: Any) -> None:
        ...
        assert hasattr(approval_request, "description"), "Approval request should have description"
        assert hasattr(approval_request, "run_id"), "Approval request should have run_id"
        assert len(approval_request.description.strip()) > 0, "Approval request description should not be empty"
    ...
    def assert_workflow_cost(result: WorkflowResult, max_cost: float) -> None:
        ...
        assert result.cost <= max_cost, f"Workflow cost ${result.cost:.4f} exceeded limit ${max_cost:.4f}"
    ...
    def assert_mcp_tools_used(result: WorkflowResult, min_tools: int = 1) -> None:
        ...
        assert len(mcp_tools) >= min_tools, f"Expected at least {min_tools} MCP tools, got {len(mcp_tools)}"

    @staticmethod
    def assert_workflow_state_consistency(result1: WorkflowResult, result2: WorkflowResult) -> None:
        ...
        """
        assert result1.run_id == result2.run_id, f"Workflow IDs should match: {result1.run_id} vs {result2.run_id}"
        assert result1.status == result2.status, f"Workflow statuses should match: {result1.status} vs {result2.status}"
