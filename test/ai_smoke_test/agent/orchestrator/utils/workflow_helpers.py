...
class WorkflowTestHelper:
    ...
    async def wait_for_workflow_completion(
        ...
        # Timeout reached
        raise TimeoutError(
            f"Workflow {run_id} did not complete within {timeout_seconds} seconds. Last status: {current_result.status}"
        )
    ...
    async def run_and_wait_for_completion(
        ...
        initial_result: WorkflowResult = await self.orchestrator_service.run_workflow(
            task_description=task_description, agent_role=agent_role, timeout_seconds=timeout_seconds
        )

        # Wait for completion
        return await self.wait_for_workflow_completion(
            initial_result=initial_result, timeout_seconds=timeout_seconds, poll_interval=poll_interval
        )
    ...
    def verify_langgraph_execution_flow(self, result: WorkflowResult) -> None:
        ...
        assert result.status == WorkflowStatus.SUCCESS, (
            f"Workflow must complete successfully. Current status: {result.status}"
        )
    ...
    async def verify_workflow_state_consistency(self, run_id: str) -> None:
        ...
        assert status1.status == status2.status, f"Workflow status inconsistent: {status1.status} vs {status2.status}"

        # Workflow ID should be consistent
        assert status1.run_id == status2.run_id == run_id, "Workflow ID inconsistent across status checks"
