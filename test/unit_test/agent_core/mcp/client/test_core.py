...
from gearmeshing_ai.agent_core.mcp.client.core import MCPClient, EasyMCPClient, AsyncMCPClient, ClientStats
from gearmeshing_ai.agent_core.mcp.client.config import MCPClientConfig, RetryConfig
from gearmeshing_ai.agent_core.mcp.client.transports import SSETransport, HTTPTransport
from gearmeshing_ai.agent_core.mcp.client.exceptions import ConnectionError, TimeoutError, ServerError
...
class TestMCPClientRetryLogic:
    ...
    async def test_retry_on_connection_error(self):
        ...
        mock_transport.list_tools.side_effect = [
...
class TestClientStatistics:
    ...
    async def test_stats_updated_on_failure(self):
        ...
        config = MCPClientConfig(
            retry_policy=RetryConfig(max_retries=0, base_delay=0.1)
        )
        client = MCPClient(config)
        mock_transport = AsyncMock()
