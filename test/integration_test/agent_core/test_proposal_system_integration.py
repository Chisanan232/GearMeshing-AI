...
class MockMCPClient(MCPClientAbstraction):
    ...
    def __init__(self):
        ...
        self.tool_catalog = MCPToolCatalog(tools=[
            MCPToolInfo(
                name="run_tests",
...
class TestProposalSystemIntegration:
    ...
    @patch('gearmeshing_ai.agent_core.adapters.pydantic_ai.PydanticAgent')
    @patch('gearmeshing_ai.agent_core.adapters.pydantic_ai.OpenAIModel')
    async def test_traditional_agent_creation(self, mock_openai, mock_pydantic_agent, traditional_factory, agent_settings):
    ...
    async def test_complete_proposal_workflow(self, proposal_factory, agent_settings):
        ...
        proposal_factory.adapter.run = AsyncMock(return_value=ActionProposal(
            action="run_tests",
            parameters={"test_type": "unit"},
            reason="Need to test"
        ))
        ...
        # Run complete task
        result = await proposal_factory.run_proposal_task(
            role="test_agent",
            task="Run the tests",
            context={}
        )
    ...
    @patch('gearmeshing_ai.agent_core.adapters.pydantic_ai.PydanticAgent')
    @patch('gearmeshing_ai.agent_core.adapters.pydantic_ai.OpenAIModel')
    async def test_proposal_vs_traditional_caching(self, mock_openai, mock_pydantic_agent, proposal_factory, traditional_factory, agent_settings):
