...
from gearmeshing_ai.agent_core.abstraction import (
    AgentFactory, AgentAdapter, MCPClientAbstraction, 
    AgentCache, AgentSettings, ModelSettings, EnvManager
)
        ...
        if agent.tools:
            for tool in agent.tools[:2]:  # Use up to 2 tools
                if hasattr(tool, 'execute'):
                    ...
                    execution_record["tools_used"].append({
                        "tool": tool,
                        "result": result
                    })
...
class TestCompleteWorkflowIntegration:
    ...
    async def test_streaming_workflow_with_multiple_agents(self, factory, agent_configurations):
        ...
        for role, chunks in streaming_results.items():
            ...
            assert f"Response from {role}:" in full_response, f"Full response '{full_response}' does not contain 'Response from {role}:'"
    ...
    async def test_workflow_with_environment_integration(self):
        ...
        with patch.dict('os.environ', {}, clear=True), \
            ...
            model_settings = ModelSettings(
                customized_name="env-test-model",
                provider="openai",
                model="gpt-4"
            )
            agent_settings = AgentSettings(
                role="env-test-agent",
                description="Environment integration test agent",
