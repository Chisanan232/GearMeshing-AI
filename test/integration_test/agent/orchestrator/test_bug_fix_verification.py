...
class TestOrchestratorBugFix:
    ...
    async def test_workflow_creation_no_parameter_error(self) -> None:
        ...
        result = await service.run_workflow(
            task_description="Test task", agent_role="dev", user_id="test_user", timeout_seconds=30
        )
    ...
    async def test_workflow_state_validation_works(self) -> None:
        ...
        result = await service.run_workflow(
            task_description="Test task for state validation", agent_role="dev", user_id="test_user", timeout_seconds=30
        )
    ...
    async def test_complete_workflow_execution(self) -> None:
        ...
        result = await service.run_workflow(
            task_description="Complete workflow test", agent_role="dev", user_id="test_user", timeout_seconds=60
        )
    ...
    async def test_approval_workflow_execution(self) -> None:
        ...
        with (
            patch.object(service.persistence, "load_workflow_state") as mock_load,
            ...
            initial_result = await service.run_workflow(
                task_description="Approval workflow test", agent_role="dev", user_id="test_user", timeout_seconds=30
            )
    ...
    async def test_workflow_error_handling(self) -> None:
        ...
        result = await service.run_workflow(
            task_description="Error handling test", agent_role="dev", user_id="test_user", timeout_seconds=30
        )
