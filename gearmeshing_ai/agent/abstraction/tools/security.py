...
def validate_file_path(file_path: str, operation: str) -> ValidationResult:
    ...
    try:
        if not file_path:
            return ValidationResult(valid=False, error="File path cannot be empty")

        # Check for directory traversal attempts before resolving
        if ".." in file_path:
            return ValidationResult(valid=False, error="Directory traversal not allowed")
        ...
        if path.suffix.lower() in dangerous_extensions and operation == "write":
            return ValidationResult(valid=False, error=f"File extension {path.suffix} not allowed for write operations")
        ...
        return ValidationResult(valid=False, error=f"Path validation error: {e!s}")
...
def validate_command(command: str) -> ValidationResult:
    ...
    try:
        if not command:
            return ValidationResult(valid=False, error="Command cannot be empty")
        ...
        for dangerous in dangerous_commands:
            if dangerous in command_lower:
                return ValidationResult(valid=False, error=f"Command '{dangerous}' is not allowed")

        # Check for shell injection attempts
        dangerous_patterns = ["&&", "||", ";", "|", "`", "$(", ">", ">>", "<"]
        for pattern in dangerous_patterns:
            if pattern in command:
                return ValidationResult(valid=False, error=f"Command pattern '{pattern}' not allowed")
        ...
        return ValidationResult(valid=False, error=f"Command validation error: {e!s}")
