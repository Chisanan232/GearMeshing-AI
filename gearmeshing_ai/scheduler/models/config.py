...
class SchedulerTemporalConfig(BaseSchedulerModel):
    ...
    worker_poll_timeout: timedelta = Field(
        default=timedelta(seconds=30),
        description="Worker poll timeout"
    )
    
    # Client configuration
    client_timeout: timedelta = Field(
...
class SchedulerMonitoringConfig(BaseSchedulerModel):
    ...
    max_concurrent_checks: int = Field(
        default=10,
        ge=1,
        description="Maximum number of concurrent checking point evaluations"
    )
    
    check_timeout_seconds: int = Field(
        default=60,
        ge=5,
        description="Timeout for individual checking point evaluations"
    )
    ...
    max_error_rate: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Maximum error rate before stopping monitoring"
    )
    
    error_cooldown_seconds: int = Field(
        default=300,
        ge=60,
        description="Cooldown period after high error rate"
    )
...
class SchedulerConfig(BaseSchedulerModel):
    ...
    temporal: SchedulerTemporalConfig = Field(
        default_factory=SchedulerTemporalConfig,
        description="Temporal integration configuration"
    )
    
    # Monitoring configuration
    monitoring: SchedulerMonitoringConfig = Field(
        default_factory=SchedulerMonitoringConfig,
        description="Monitoring configuration"
    )
    
    # Logging configuration
    logging: SchedulerLoggingConfig = Field(
        default_factory=SchedulerLoggingConfig,
...
class MonitorConfig(BaseSchedulerModel):
    ...
    description: str = Field(
        default="",
        description="Description of what this monitoring configuration does"
    )
    ...
    checking_points: List[Dict[str, Any]] = Field(
        ...
        description="List of checking point configurations"
    )
    ...
    def get_checking_points_by_type(self, cp_type: str) -> List[Dict[str, Any]]:
        ...
        """
        return [
            cp for cp in self.checking_points 
            if cp.get("type") == cp_type and cp.get("enabled", False)
        ]
