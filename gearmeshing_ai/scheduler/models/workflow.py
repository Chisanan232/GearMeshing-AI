...
class AIAction(BaseSchedulerModel):
    ...
    prompt_template_id: Optional[str] = Field(
        None,
        description="ID of the prompt template to use for this action"
    )
    ...
    agent_role: Optional[str] = Field(
        None,
        description="Role of the AI agent that should execute this action"
    )
    
    approval_required: bool = Field(
        default=False,
        description="Whether this action requires approval before execution"
    )
    
    approval_timeout_seconds: int = Field(
        default=3600,
        ge=60,
        description="Timeout for approval in seconds"
    )
    ...
    priority: int = Field(
        ...
        ge=1,
        le=10,
        description="Priority of this action (1=lowest, 10=highest)"
    )
    
    scheduled_at: Optional[datetime] = Field(
        None,
        description="When this action should be scheduled for execution"
    )
    ...
    parent_execution_id: Optional[str] = Field(
        None,
        description="ID of the parent execution, if this is a child action"
    )
...
class AIWorkflowInput(BaseSchedulerModel):
    ...
    parent_workflow_id: Optional[str] = Field(
        None,
        description="ID of the parent workflow, if this is a child workflow"
    )
    
    # Execution options
    dry_run: bool = Field(
        default=False,
        description="Whether to run in dry-run mode (no actual actions)"
    )
    
    debug_mode: bool = Field(
        default=False,
        description="Whether to run in debug mode with additional logging"
    )
...
class AIWorkflowResult(BaseSchedulerModel):
    ...
    completed_at: Optional[datetime] = Field(None, description="When the workflow execution completed")
    duration_ms: Optional[int] = Field(None, description="Duration of execution in milliseconds")
    ...
    actions_taken: List[str] = Field(
        ...
        description="List of actions that were taken"
    )
    ...
    data_summary: Dict[str, Any] = Field(
        default_factory=dict,
        description="Summary of the data that was processed"
    )
    
    # Approval information
    approval_required: bool = Field(
        default=False,
        description="Whether approval was required for this execution"
    )
    
    approval_granted: bool = Field(
        default=False,
        description="Whether approval was granted (if required)"
    )
    
    approval_details: Dict[str, Any] = Field(
        default_factory=dict,
