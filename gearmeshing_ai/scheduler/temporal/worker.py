...
from gearmeshing_ai.scheduler.workflows import AIWorkflowExecutor, SmartMonitoringWorkflow
...
class TemporalWorker:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self._running = False
            raise RuntimeError(f"Failed to start Temporal worker: {e!s}")
    ...
    def _create_sandbox_restrictions(self) -> SandboxRestrictions | None:
...
class WorkerManager:
    ...
    async def start_all(self) -> None:
        ...
        for name, worker in self._workers.items():
            task = asyncio.create_task(self._run_worker_with_error_handling(name, worker), name=f"worker-{name}")
            tasks.append(task)
    ...
    async def stop_all(self) -> None:
        ...
        tasks = []
        for name, worker in self._workers.items():
            task = asyncio.create_task(self._stop_worker_with_error_handling(name, worker), name=f"stop-worker-{name}")
            tasks.append(task)
    ...
    async def _run_worker_with_error_handling(self, name: str, worker: TemporalWorker) -> None:
        ...
        try:
            ...
            print(f"Worker {name} failed: {e!s}")
    ...
    async def _stop_worker_with_error_handling(self, name: str, worker: TemporalWorker) -> None:
        ...
        try:
            ...
            print(f"Failed to stop worker {name}: {e!s}")
