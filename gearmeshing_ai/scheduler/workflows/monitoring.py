...
from gearmeshing_ai.scheduler.models.workflow import AIAction, AIWorkflowInput, AIWorkflowResult
...
class SmartMonitoringWorkflow(BaseWorkflow):
    ...
    async def run(self, monitor_config: MonitorConfig) -> None:
        ...
        try:
            ...
            enabled_checking_points = {
                name: cp for name, cp in checking_points.items() 
                if cp.enabled
            }
            ...
            while True:
                try:
                    ...
                    for data_item in data_items:
                        await self.process_data_item_with_checking_points(
                            data_item, enabled_checking_points
                        )
    ...
    async def process_data_item_with_checking_points(
        self, 
        data_item: MonitoringData, 
        checking_points: Dict[str, Any]
    ) -> None:
