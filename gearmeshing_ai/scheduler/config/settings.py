...
class SchedulerSettings(BaseSettings):
    ...
    monitoring_max_concurrent_checks: int = Field(
        default=10, ge=1, description="Maximum concurrent checking point evaluations"
    )
    monitoring_check_timeout_seconds: int = Field(
        default=60, ge=5, description="Timeout for individual checking point evaluations"
    )

    # Configuration file
    config_file: Path | None = Field(None, description="Path to scheduler configuration file")

    # Security configuration
    secret_key: SecretStr | None = Field(None, description="Secret key for authentication and encryption")
    ...
    log_file_dir: Path | None = Field(None, description="Directory for log files")

    # Database configuration
    database_url: SecretStr | None = Field(None, description="Database connection URL")
    database_pool_size: int = Field(default=5, ge=1, description="Database connection pool size")

    # Redis configuration
    redis_url: SecretStr | None = Field(None, description="Redis connection URL")
    redis_max_connections: int = Field(default=10, ge=1, description="Maximum Redis connections")

    # MCP Gateway configuration
    mcp_gateway_url: str | None = Field(None, description="MCP Gateway URL")
    mcp_gateway_token: SecretStr | None = Field(None, description="MCP Gateway authentication token")

    # External service configuration
    clickup_api_token: SecretStr | None = Field(None, description="ClickUp API token")
    slack_bot_token: SecretStr | None = Field(None, description="Slack bot token")
    slack_user_token: SecretStr | None = Field(None, description="Slack user token")
    ...
    google_api_key: SecretStr | None = Field(None, description="Google AI API key")
    ...
    def get_mcp_gateway_token(self) -> str | None:
    ...
    def get_clickup_api_token(self) -> str | None:
    ...
    def get_slack_user_token(self) -> str | None:
...
_scheduler_settings_instance: SchedulerSettings | None = None
