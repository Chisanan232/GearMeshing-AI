...
class CheckingPointRegistry:
    ...
    def unregister(self, name: str) -> None:
        ...
        if cp_type and cp_type in self._type_mapping:
            self._type_mapping[cp_type] = [name for name in self._type_mapping[cp_type] if name != name]
    ...
    def get_class(self, name: str) -> type[CheckingPoint] | None:
    ...
    def get_instance(self, name: str, config: dict[str, Any] | None = None) -> CheckingPoint | None:
    ...
    def get_all_instances(self, configs: dict[str, dict[str, Any]] | None = None) -> dict[str, CheckingPoint]:
    ...
    def get_by_type(self, cp_type: str | CheckingPointType) -> list[str]:
    ...
    def get_enabled_instances(self, configs: dict[str, dict[str, Any]] | None = None) -> dict[str, CheckingPoint]:
        ...
        return {name: instance for name, instance in all_instances.items() if instance.enabled}

    def validate_all(self, configs: dict[str, dict[str, Any]] | None = None) -> dict[str, list[str]]:
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "type_counts": {cp_type.value: len(names) for cp_type, names in self._type_mapping.items()},
            "checking_points": {
                name: {
                    "name": name,
                ...
                for importer, modname, ispkg in pkgutil.iter_modules(package.__path__, package.__name__ + "."):
                    try:
                        ...
                        for name, obj in inspect.getmembers(module, inspect.isclass):
                            if (
                                obj != CheckingPoint
                                and issubclass(obj, CheckingPoint)
    ...
    def initialize(self, auto_discover_packages: list[str] | None = None) -> None:
        """Initialize the registry.
...
def get_checking_point(name: str, config: dict[str, Any] | None = None) -> CheckingPoint | None:
    """Get a checking point instance by name.
...
def get_all_checking_points(configs: dict[str, dict[str, Any]] | None = None) -> dict[str, CheckingPoint]:
    """Get all checking point instances.
...
def get_checking_points_by_type(
    cp_type: str | CheckingPointType, configs: dict[str, dict[str, Any]] | None = None
) -> dict[str, CheckingPoint]:
...
def initialize_registry(auto_discover_packages: list[str] | None = None) -> None:
