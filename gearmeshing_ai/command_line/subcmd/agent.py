...
def list(
    status: Optional[str] = typer.Option(
        None, "--status", "-s", help="Filter by agent status"
    ),
    limit: int = typer.Option(
        10, "--limit", "-l", help="Maximum number of agents to list"
    ),
) -> None:
    """List all agents."""
...
def create(
    ...
    config_file: Optional[Path] = typer.Option(
        None, "--config", "-c", help="Path to agent configuration file"
    ),
) -> None:
    """Create a new agent."""
...
def run(
    ...
    input_text: str = typer.Option(
        None, "--input", "-i", help="Input text for the agent"
    ),
    input_file: Optional[Path] = typer.Option(
        None, "--file", "-f", help="Input file for the agent"
    ),
    interactive: bool = typer.Option(
        False, "--interactive", help="Run in interactive mode"
    ),
) -> None:
    """Run an agent."""
...
def stop(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Force stop the agent"
    ),
) -> None:
