...
class MonitoringConfig(BaseModel):
    ...
    metrics_interval: float = Field(
        default=30.0, ge=1.0, le=300.0, description="Metrics collection interval in seconds"
    )
    enable_health_checking: bool = Field(default=True, description="Enable health checking")
    health_check_interval: float = Field(
        default=60.0, ge=10.0, le=600.0, description="Health check interval in seconds"
    )
...
class MCPClientConfig(BaseModel):
    ...
    auth_token: SecretStr | None = Field(default=None, description="Authentication token")
    ...
    @classmethod
    def from_file(cls, file_path: str | Path) -> "MCPClientConfig":
