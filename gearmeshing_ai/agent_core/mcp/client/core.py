...
class EasyMCPClient:
    ...
    async def call_tool_sse(
        url: str, tool_name: str, arguments: dict[str, Any] | None = None, timeout: float = 30.0
    ) -> Any:
...
class AsyncMCPClient(MCPClient):
    ...
    async def list_tools(self) -> list[str]:
        ...
        return await self._execute_with_concurrency_limit(self._transport.list_tools, "list_tools")
    ...
    async def call_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> Any:
        ...
        return await self._execute_with_concurrency_limit(
            lambda: self._transport.call_tool(tool_name, arguments), f"call_tool({tool_name})"
        )
