...
class AgentFactory:
    ...
    def __init__(self, adapter: AgentAdapter, mcp_client: Optional[MCPClientAbstraction] = None):
    ...
    def get_agent_settings(self, role: str) -> Optional[AgentSettings]:
        return self._agent_settings_registry.get(role)

    def get_model_settings(self, customized_name: str) -> Optional[ModelSettings]:
        return self._model_settings_registry.get(customized_name)

    async def get_or_create_agent(self, role: str, override_settings: Optional[Dict[str, Any]] = None) -> Any:
        """
